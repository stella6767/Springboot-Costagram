...
public interface ImageRepository extends JpaRepository<Image, Integer> {
	...
	//@Query(value = "select * from image where id in (select imageId from (select imageId, count(imageId) likeCount from likes group by imageId order by 2 desc) t) and where userId = :principalId" , nativeQuery = true)
